...
class SlidingWindowUnitTests extends AnyFlatSpec
...
with BeforeAndAfterEachTestData {
  ...
  it should "show appropirate window as it cycles through the input image with a bigger window" in {
    test(new SlidingWindowUnitTestBed(kernelSize = 3,
                                      kernelDepth = 2,
                                      actWidth = 5,
                                      actHeight = 4,
                                      actParamSize = 6,
          ...
          //                  data                      rowAddr, ChAddr, rowWriteMode
          val testVec = Seq(("b000011_000010_000001".U(18.W), 0, 0,  true),  // (1, 2, 3)
                            ...
                            ("b101000_100011_011110".U(18.W), 2, 1,  false)) // (30, 35, 40)
  ...
  for (testcaseId <- 0 until 10) {
    ...
    it should s"""testcaseid: $testcaseId, random params: kernelSize=$randKernelSize, kernelDepth=$randKernelDepth
      ...
      test(new SlidingWindowUnitTestBed(kernelSize = randKernelSize,
        ...
        for (i <- 0 until randImageHeight - randKernelSize + 1) {
          for (j <- 0 until randImageWidth - randKernelSize + 1) {
            val window = randImage.get(NDArrayIndex.all(),  // kernel
