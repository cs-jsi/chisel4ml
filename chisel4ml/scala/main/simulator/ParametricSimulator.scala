...
/** A simulator that can be customized by passing some parameters through a list of [[SimulatorSettings]]. It offers the
  * possibility to:
...
class ParametricSimulator {
  ...
  /** Set settings before the simulation starts. It sets the initial settings such as the backend compile settings,
    * trace style, etc.
    */
  ...
  /** Set the controller settings. It sets settings such as the trace output enable.
    */
  ...
  protected class DefaultParametricSimulator(val workspacePath: String, val tag: String)
    ...
    /** Cleanup the simulation and move the simulation workspace to the wanted workspace path.
      */
    def cleanup(): Unit = {
